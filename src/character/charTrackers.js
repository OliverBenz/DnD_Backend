...
exports.patchCharTracker = function(req, res){
  db.query("UPDATE charTrackers SET value = " + req.body.trackValue + " WHERE trackerId = " + req.body.id + " AND charId = (SELECT id FROM characters WHERE charString = " + req.params.charString + ")", (result) => {
    if(result["success"]) res.status(200);
    else{
      res.status(500);
