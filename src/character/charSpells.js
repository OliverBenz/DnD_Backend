...
exports.checkCharSpell = function(req, res){
  db.query("SELECT * FROM charSpells WHERE characterId = (SELECT id FROM characters WHERE charString = '" + req.params.charString + "') AND spellId = " + req.params.spellId, (result) => {
    if(result["success"]){
      res.status(200);
      
      if(result["data"].length === 1) result["data"] = true;
